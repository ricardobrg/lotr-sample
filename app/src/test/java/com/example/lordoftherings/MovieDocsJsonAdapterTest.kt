...
import com.example.lordoftherings.movie.data.MovieModelList
...
class MovieDocsJsonAdapterTest {
    ...
    @Test
    fun movieDocsJsonAdapter_canParseJsonToMovieModelList() {
        val moshi: Moshi = Moshi.Builder()
            .add(KotlinJsonAdapterFactory())
            .build()
        val adapter: JsonAdapter<MovieModelList> = moshi.adapter(MovieModelList::class.java)
        val movieModelList: MovieModelList? = adapter.fromJson(jsonString)
        movieModelList?.docs?.isNotEmpty()?.let { Assert.assertTrue(it) }
        Assert.assertTrue(movieModelList!!.docs.size == movieModelList.total)
    }
}